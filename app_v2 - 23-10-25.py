...
def generate_team_checklist(cur, g_id, team):
    ...
    if team in team_tasks:
        for task in team_tasks[team]:
            cur.execute("""
                INSERT INTO bid_checklists (g_id, task_name, description, priority, status, created_by)
                VALUES (%s, %s, %s, %s, 'pending', %s)
            """, (g_id, task['name'], task['description'], task['priority'], current_user.id))
...
def team_dashboard(team):
    ...
    cur.execute("""
        ...
        FROM go_bids 
        WHERE LOWER(COALESCE(state, 'analyzer')) = %s
    """, (current_stage,))
...
def team_employees(team):
    ...
    # Get team leads (users with this role)
...
def create_checklist_item(team, g_id):
    ...
    try:
        ...
        cur = mysql.connection.cursor()
        cur.execute("""
            INSERT INTO bid_checklists (g_id, task_name, description, assigned_to, priority, due_date, created_by)
            VALUES (%s, %s, %s, %s, %s, %s, %s)
        """, (g_id, task_name, description, assigned_to if assigned_to else None, 
              priority, due_date if due_date else None, current_user.id))
...
def transfer_project(team):
    ...
    try:
        ...
        cur.execute("""
            UPDATE bid_checklists 
            SET status = 'completed', team_archive = %s, updated_at = CURRENT_TIMESTAMP
...
def api_bid_tasks(team, g_id):
    ...
    # First check if this bid belongs to the current team
    cur.execute("SELECT state FROM go_bids WHERE g_id = %s", (g_id,))
    bid = cur.fetchone()
    
    if not bid or bid['state'] != team:
        cur.close()
        return jsonify({'tasks': []})  # Return empty if bid doesn't belong to this team
    ...
    cur.execute("""
        ...
        WHERE bc.g_id = %s 
        AND (
            (u.role = %s AND bc.team_archive IS NULL) OR  -- Current team's active tasks
